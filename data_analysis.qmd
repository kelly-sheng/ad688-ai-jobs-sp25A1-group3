...
# Use the already-cleaned frame if present, else load
try:
    df = raw_df.copy()
except NameError:
    df = pd.read_csv("data/lightcast_job_postings.csv")

# Pick a text column robustly
...
def _clean_text_input(x):
    """
    Accepts Series, 1-col DataFrame, or numpy array from ColumnTransformer
    and returns a plain Python list[str] with NaNs -> "".
    """
    if isinstance(x, pd.Series):
        ...
        # if 2-D (n,1) or (1,n), flatten to (n,)
        if x.ndim == 2:
            if x.shape[1] == 1:
                s = pd.Series(x[:, 0])
            else:
    ...
    return s.tolist()   # ensure each doc is a plain string, not an array

# Handle OneHotEncoder API difference
try:
...
pre = ColumnTransformer(
    transformers=[
        ("txt", text_pipe, [text_col]),  # keep as list so CT passes a 2-D slice
...
# Build text-only matrix from cleaned text
...
plt.scatter(XY[:,0], XY[:,1], c=labels, s=6, alpha=0.5, cmap="tab10")
plt.title("KMeans clusters (2-D SVD embedding)")
plt.xlabel("SVD 1"); plt.ylabel("SVD 2")
plt.tight_layout(); plt.savefig("output/kmeans_svd_scatter.png", dpi=150); plt.show()
