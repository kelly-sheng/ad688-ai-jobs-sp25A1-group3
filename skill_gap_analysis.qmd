...
# expected_per_skill (Series: index=skills, values=industry target 1–5)
...
# Order columns: Skill, each person…, Team Avg, Target, Gap
...
for skill in top_skills:
    row = {
        ...
        **{name: float(df_team_top10.loc[name, skill]) for name in members},
        ...
        "Gap (Target−Avg)": round(float(gap[skill]), 2) if pd.notna(gap[skill]) else np.nan,
    }
    rows.append(row)
...
comparison_table = comparison_table.sort_values("Gap (Target−Avg)", ascending=False, na_position="last").reset_index(drop=True)
...
plt.title("Team vs. Industry Expertise Demand — Top Skills (0–5)")
